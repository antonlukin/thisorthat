...
Поле `total` типа `Integer` содержит общее количество вопросов в выборке. Например, чтобы получить все 247 вопросов пользователя, 
нужно сделать три запроса по 100 вопросов со смещением 0, 100 и 200 соответственно.
...
Ответ сервера содержит список объектов `comments`.  
Поле `total` типа `Integer` отображает общее количество вопросов в выборке. 
...
| **token**   | `^\d+:[a-z0-9]{32}$` | Ключ авторизации, полученный при регистрации. |
| **item_id** | `^\d{1,11}$`         | Идентификатор вопроса, на который отправляется жалоба. |
| **text**    | `^.{1,300}$`         | Текст комментария длинной 1-300 символов. <br>К значению применяется фильтр антимата. |
...
```bash
curl --include --request POST --data 'item_id=1&text=Отличный комментарий' 'https://api.thisorthat.ru/addComment?token=1:0994f52572ab3f9432c77615c104db9c'
```
...
Ответ сервера содержит `comment_id` типа `String` добавленного комментария. В случае ошибки клиент может отобразить на своей стороне результат поля `description`.
Между запросами на добавление комментария должно пройти минимум 10 секунд, иначе сервер вернет ошибку.