...
| **token**   | `^\d+:[a-z0-9]{32}$`      | Ключ авторизации, полученный при регистрации. |
| **item_id** | `^\d{1,11}$`              | Идентификатор вопроса, на который отправляется жалоба. |
| limit       | `^[1-9][0-9]?$\|100$`     | Количество запрашиваемых комментариев в диапазоне 1-100. <br>По умолчанию равен _50_ |
| offset      | `^[0-9]+$`                | Смещение относительно первого комментария. <br>По умолчанию равен _0_ |
...
Между запросами на добавление комментария должно пройти хотя бы 10 секунд. Иначе сервер вернет ошибку.
