...
Ответ сервера содержит `item_id` добавленного вопроса. В случае ошибки клиент может отобразить на своей стороне результат поля `description`.
...
содержащее параметр `clone` с id похожего вопроса.
> Если поле `parameters.clone` не пустое, клиент может предложить добавить вопрос в избранное.

## /setViewed

Метод для добавления ответов пользователя. Позволяет отправить массив одни запросом.
...
```bash
curl --include --request POST --data 'views[1]=first&views[2]=last&views[3]=skip' 'https://api.thisorthat.ru/setViewed?token=1:0994f52572ab3f9432c77615c104db9c'
```
